module Day24.Part2 (thisMain) where
...
thisMain = do
    ...
    let valleyBounds = Bounds
            ...
            startPos
            endPos
    let allBlizzies = iterate (moveBlizzards valleyBounds) valley
    let leaveValley = exitMins
            endPos
            valleyBounds
            (head allBlizzies)
            startPos
    let gettingSnackies = exitMins
            startPos
            valleyBounds
            (allBlizzies !! leaveValley)
            endPos
    let bringingSnackies = exitMins
            endPos
            valleyBounds
            (allBlizzies !! (leaveValley + gettingSnackies))
            startPos
    print $ bringingSnackies + gettingSnackies + leaveValley
    hClose handle
